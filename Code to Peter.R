...
#visual 
plot(out.np,col="red",alternate.chrid = TRUE,ylim = c(0,1.5))
plot(out1.em1,col="green",lty=4,add = TRUE)

plot(out.np,col="red",alternate.chrid = TRUE,ylim = c(0,3))
plot(out1.em1,col="blue",lty=3,add = TRUE)
plot(out.cim,col="yellow",lty=2,add=TRUE)
plot(out.np - out1.em1, ylim=c(-0.5, 0.5), ylab=expression(LOD[HK] - LOD[EM]))
abline(h=0,lty=3, col="red")
plot(out.np - out1.em1, chr=c("C3", "C7"), ylim=c(-0.5, 0.5), ylab=expression(LOD[HK] - LOD[EM]))
abline(h=0, lty=3,col="red")

plot(out1.em1, out.np, ylab="LOD score", lty=c(1,2))
plot(out.np-out1.em1,ylab=expression(LOD[NP-EM]),ylim=c(-0.2,0.3))
abline(h=0,lty=3,col="red")
plot(out.np-out1.em1, out.cim-out1.em1, col=c( "red","green"), ylim=c(-1.8,1.8), ylab=expression(LOD[NP & CIM] - LOD[EM]))
